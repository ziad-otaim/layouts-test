...
const AboutComponent = ({ subdomain, params }) => {
  if (subdomain.type === "college") {
    if (params.length >= 1) {
      return <DepartmentAbout department={params[0]} />;
    } else {
      return <CollegeAbout college={subdomain.name} />;
    }
  }
  return <UniAbout />;
};
...
const MainAboutPage = () => {
  ...
  return (
    <div style={{ padding: "2rem" }}>
      <AboutComponent subdomain={subdomain} params={params} />
    </div>
  );
};
